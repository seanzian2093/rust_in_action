...
// Operation and Orientation enum types are defined later.
// Including these with the use keyword removes a lot of noise from source code - how?
use crate::Operation::{
    Forward,
    Home,
    Noop,
...
// riddance of temp var steps
// provide more opportunities for Rust compiler to optimize code execution
// Iterators are efficient abstraction - working with their methods directly allows compiler to create
// optimal code that takes minimal memory
// Use rayon to add data parallelism to code, i.e., applying same function/closure on different data, Vec<T>
// rayon guarantees that there is no race condition
// When iterator is not available, use threading
